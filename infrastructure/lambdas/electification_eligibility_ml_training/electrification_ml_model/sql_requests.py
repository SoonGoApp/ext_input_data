...
masternaut_vehicles AS (
    ...
    INNER JOIN common.synchronisations s ON s.id = m.synchronisation_id
    WHERE s.connector_id = (
        SELECT id FROM common.connectors WHERE name = 'MASTERNAUT'
    )
...
base_vehicles AS (
    SELECT
        ...
        CASE WHEN UPPER(TRIM(COALESCE(v.energy, ''))) = 'ELECTRIC' THEN 1 ELSE 0 END AS is_electric
    FROM publ.vehicles v
    INNER JOIN masternaut_vehicles mv ON mv.vehicle_id = v.id
    ...
    WHERE v.fiscal_type = 'PERSONAL_CAR'
      ...
      AND v.organization_id = (SELECT id FROM common.organizations WHERE slug = 'acorus')
),
...
daily_km_calculated AS (
    SELECT
        ...
        CASE 
            ...
            THEN (mileage - prev_mileage)::numeric / NULLIF((mileage_date::date - prev_date::date), 0)
            ELSE NULL
        END AS daily_km_raw
    FROM mileage_daily_km
),

daily_km_filtered AS (
    SELECT
        vehicle_id,
        CASE 
            WHEN daily_km_raw <= 2000 THEN daily_km_raw
            ELSE NULL
        END AS daily_km
    FROM daily_km_calculated
),

target_mileage AS (
    SELECT
        vehicle_id,
        COALESCE(MAX(daily_km), 0) AS max_daily_km
    FROM daily_km_filtered
    GROUP BY vehicle_id
),

target_with_flags AS (
    SELECT
        tm.vehicle_id,
        CASE WHEN tm.max_daily_km > 300.0 THEN 1 ELSE 0 END AS target_mileage
