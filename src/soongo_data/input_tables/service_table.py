...
from soongo_data.data_models import (SoonGoRecordsModel,
                                     VehicleMaintenanceModel, VehiclesModel)
from soongo_data.sql_mappings import VehicleServicesTable, VehiclesTable
...
@add_input_table
class ServicesInputTable(InputTable):

    def __init__(self: typing.Self):
        super().__init__(
            sql_mapping=VehicleServicesTable,
            columns=[
                ...
                InputColumn.from_data_column(
                    column=VehicleMaintenanceModel.computed_service_mileage,
                    nullable=True,
                    sql_name='mileage',
                ),
                InputColumn.from_data_column(
                    column=VehicleMaintenanceModel.computed_service_date,
                    nullable=False,
                    is_id=True,
                ),
                InputColumn.from_data_column(
                    column=VehicleMaintenanceModel.maintenance_date,
                    sql_name='effective_service_date',
                ),
                InputColumn.from_data_column(
                    column=VehicleMaintenanceModel.scheduled_service_date,
                ),
                InputColumn.from_data_column(
    ...
    def _get_df(
        ...
        return combined_df.loc[
            ...
            &
            ~series_is_nan(combined_df[VehicleMaintenanceModel.computed_service_date.name]),
    ...
    def to_sql(
        ...
        service_query = sa.select(
            VehicleServicesTable.id,
            VehiclesTable.plate_number,
            VehicleServicesTable.effective_service_date.label(
                'db_effective_service_date'
            ),
            VehicleServicesTable.scheduled_service_date.label(
                'db_scheduled_service_date'
            ),
            VehicleServicesTable.computed_service_date.label(
                'db_computed_service_date'
            ),
        ).select_from(
            VehicleServicesTable
        ).join(
            VehiclesTable,
            VehicleServicesTable.vehicle_id == VehiclesTable.id,
        ).filter(
            VehicleServicesTable.organization_id == organization_id
        )
        ...
        computed_date_only = (
            db_df['db_effective_service_date'].isna()
            &
            db_df['db_scheduled_service_date'].isna()
        )
        ...
        delete_query = sa.delete(
            ...
            VehicleServicesTable.id.in_(computed_only['id'])
        )
        ...
        session.execute(
            sa.text('refresh materialized view publ.services_view;')
        )
...
if __name__ == "__main__":
    ...
    logger = gen_logger('services_table')
    ...
    service_table = ServicesInputTable()
