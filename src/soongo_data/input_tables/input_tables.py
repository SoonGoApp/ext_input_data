...
from soongo_data.connectors import (Dataset,
                                    ConnectorData,
                                    check_no_dup_values)
...
class InputTable(ABC):
    ...
    def __init__(
        ...
        col_name_mapping: typing.Optional[dict] = None,
    ) -> None:
        ...
        self.col_name_mapping = {
            col_name: col_name for col_name
            in self.return_column_names()
        }
        self.col_name_mapping.update(col_name_mapping or {})
    ...
    def to_sql(
        ...
        sliding_cols: typing.Optional[typing.Tuple[str, str]] = None,
    ) -> pd.DataFrame:
        ...
        data_df = data_df.rename(columns=self.col_name_mapping)
        ...
        if (
            ...
            if VehiclesModel.full_model.name in id_cols:
                connector_col = id_cols.pop(
                    VehiclesModel.full_model.name
                )
                id_cols[VehiclesModel.trim_id.name] = InputColumn.from_data_column(
                    VehiclesModel.trim_id,
                    nullable=connector_col.nullable,
                )
        ...
        if (
            ...
            if SuppliersModel.supplier_id.name not in data_df:
                ...
                data_df[SuppliersModel.supplier_id.name] = supplier_inserter.insert_df(
                    data_df=data_df,
                )
        ...
        if sliding_cols:
            insert_sliding_table(
                ...
                session=session,
                organization_id=organization_id,
                start_col=sliding_cols[0],
                end_col=sliding_cols[1],
    ...
    def get_id_cols(self: typing.Self) -> typing.Dict[str, InputColumn]:
        ...
        """
        return {
            self.col_name_mapping.get(col.name, col.name): col
...
def match_soongo_employee_id(
    ...
    table_to_match[CollaboratorsModel.soongo_collab_reference.name] = gen_soongo_emp_id(
...
def subset_cols(
    ...
    elif conjunctive_cols is None:  # If not specified, return all cols
        conjunctive_cols = [{col} for col in df.columns] if df.columns else []
    required_cols = set()
