...
logger = gen_logger('Rent_Cost_Train - Data_Load')
...
class DataLoader:
    ...
    def load_data(self) -> pd.DataFrame:
        ...
        try:
                ...
                logger.info(f"Loaded {len(df)} rows from ALL_PROCESSED_DATA_SQL")
                return df
            
        except Exception as e:
            logger.error(f"Failed to load ALL_PROCESSED_DATA_SQL: {e}")
            raise 