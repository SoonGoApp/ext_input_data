...
# Colonnes utilisées pour construire la clé de groupe (voir _build_group_key).
# Pas de "model" disponible côté rent (non sélectionné en SQL) -> on groupe
# sur la signature complète des features, comme pour sell_cost.
GROUP_COLS = ['brand', 'segment']
...
class RentCostModel:
    ...
    def prepare_train_test_split(
        ...
        # Un même vehicle_id peut avoir plusieurs contrats (renouvellements,
        # avenants) -> on groupe sur la signature complète des features pour
        # éviter qu'un même véhicule / config quasi-identique se retrouve à
        # la fois en train et en test (source du data leakage initial).
    ...
    def generate_shap_analysis(
        ...
        plt.title(
            "SHAP Summary — Feature Impact on Rent Cost Prediction",
