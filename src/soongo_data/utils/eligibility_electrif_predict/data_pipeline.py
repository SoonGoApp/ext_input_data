...
logger = gen_logger('Elec_Eligibility_Predict - Data_Load')
...
class DataLoader:
    ...
    def delete_todays_rows(self, conn):
        delete_sql = f"""
            DELETE FROM publ.vehicles_electrification_eligibility_score
            WHERE created_at::date = CURRENT_DATE;
        """
        conn.execute(text(delete_sql))


    def insert_predictions_to_table(self, df):
